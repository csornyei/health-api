...
async def test_endpoint(request: Request) -> dict:
    ...
    if _TEST_DATA_FILE.exists():
        try:
            ...
            logger.error(
                "test data file is corrupt, starting fresh",
                path=str(_TEST_DATA_FILE),
                exc_info=exc,
            )
    ...
    try:
        ...
        logger.error(
            "failed to write test data file", path=str(_TEST_DATA_FILE), exc_info=exc
        )
        raise HTTPException(
            status_code=500, detail="Failed to persist test data"
        ) from exc
